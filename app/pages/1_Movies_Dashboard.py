...
c2.metric("Avg User Rating", f"{df['user_rating'].mean():.3f}")
...
if 'genre_list' in df.columns:
    ...
    c6.metric("Taste Diversity", f"{gen_entropy:.3f}", 
              help="Shannon Entropy. 0.0 means you only watch ONE genre. Maxes out around ~4.5 (if you watch a perfectly even mathematical split across all TMDB genres).")
...
fig_ratings = px.bar(rating_counts.sort_values('Rating'), x='Rating', y='Count', 
                     text='Count', color_discrete_sequence=['royalblue'])
fig_ratings.update_traces(textposition='outside', marker_line_width=1, marker_line_color="black")
...
with t_dec:
    ...
    fig = px.bar(df_decade, x='decade', y='Count', hover_data={'avg_rating': ':.3f'}, title="Movies per Decade")
...
with t_run:
    ...
    fig = px.bar(rt_agg, x='runtime_bin', y='Count', hover_data={'avg_rating': ':.3f'}, 
                 title="Runtime Distribution", color_discrete_sequence=['teal'])
    fig.update_traces(marker_line_width=1, marker_line_color="black")
    st.plotly_chart(fig, use_container_width=True)
...
with t_sea:
    if 'released' in df.columns:
        ...
        fig = px.line(season_agg, x='month', y='user_rating', markers=True,
                      title="Average Rating by Release Month", range_y=[0, 5])
        st.plotly_chart(fig, use_container_width=True)
...
def render_people_tab(col_list, label, min_count_threshold):
    ...
    with c_freq:
        ...
        fig = px.bar(freq, x='Count', y=col_list, orientation='h', color='Count', hover_data={'avg_rating': ':.3f'})
    ...
    with c_rate:
        ...
        if not rated.empty:
            fig = px.bar(rated, x='avg_rating', y=col_list, orientation='h', color='avg_rating', 
                         color_continuous_scale='Viridis', range_x=[0, 5], hover_data={'count': True})
            fig.update_layout(yaxis={'categoryorder':'total ascending'})
            fig.update_traces(marker_line_width=1, marker_line_color="black")
            st.plotly_chart(fig, use_container_width=True)
        else:
...
with t1:
    ...
    if 'rotten_tomatoes_rating' in df.columns:
        ...
        with c_rt:
            fig = px.bar(rt_agg, x='rt_bin', y='user_rating', title="Avg Rating by Rotten Tomatoes", range_y=[0, 5], color='user_rating', color_continuous_scale='RdYlGn')
            fig.update_traces(marker_line_width=1, marker_line_color="black")
            st.plotly_chart(fig, use_container_width=True)
    ...
    if 'metascore' in df.columns:
        ...
        with c_meta:
            fig = px.bar(meta_agg, x='meta_bin', y='user_rating', title="Avg Rating by Metascore", range_y=[0, 5])
            fig.update_traces(marker_line_width=1, marker_line_color="black")
            st.plotly_chart(fig, use_container_width=True)
    ...
    if 'imdb_rating' in df.columns:
        ...
        with c_imdb:
            fig = px.bar(imdb_agg, x='imdb_bin', y='user_rating', title="Avg Rating by IMDb", range_y=[0, 5])
            fig.update_traces(marker_line_width=1, marker_line_color="black")
            st.plotly_chart(fig, use_container_width=True)
    ...
    if 'vote_average' in df.columns:
        ...
        with c_tmdb:
            fig = px.bar(tmdb_agg, x='tmdb_bin', y='user_rating', title="Avg Rating by TMDB Votes", range_y=[0, 5])
            fig.update_traces(marker_line_width=1, marker_line_color="black")
            st.plotly_chart(fig, use_container_width=True)
...
with t2:
    if 'box_office' in df.columns:
        ...
        fig = px.bar(bo_agg, x='bo_tier', y='user_rating', title="Do you prefer Blockbusters or Indies?", color='user_rating', range_y=[0, 5])
        fig.update_traces(marker_line_width=1, marker_line_color="black")
        st.plotly_chart(fig, use_container_width=True)
    else:
...
with t3:
    ...
    with c_ctry:
        ...
        fig = px.bar(cnt, x='country_list', y='Count', hover_data={'avg_rating': ':.3f'}, title="Top Countries")
        fig.update_traces(marker_line_width=1, marker_line_color="black")
        st.plotly_chart(fig, use_container_width=True)
    with c_lang:
        ...
        fig = px.pie(lng, values='Count', names='language_list', hover_data={'avg_rating': ':.3f'}, title="Languages")
        fig.update_traces(marker_line_width=1, marker_line_color="black")
        st.plotly_chart(fig, use_container_width=True)
...
if 'overview' in df.columns and 'plot' in df.columns:
    ...
    fig = px.treemap(wdf, path=['Word'], values='Count', color='Count', hover_data=['Avg_Rating'], title="Dominant Themes in Your Movies")
    st.plotly_chart(fig, use_container_width=True)
...
if 'awards' in df.columns:
    ...
    c1, c2 = st.columns(2)
    with c1:
        fig = px.bar(oscar_agg, x='Label', y='user_rating', color='Label', title="Do you prefer Actual Oscar Winners?", range_y=[0, 5])
        fig.update_traces(marker_line_width=1, marker_line_color="black")
        st.plotly_chart(fig, use_container_width=True)
    with c2:
...
with c1:
    ...
    for _, row in hot_takes_good.iterrows():
        st.markdown(f"📈 **{row['title']}** (Diff: +{row['magnitude']:.2f})  \n<span style='font-size:0.85em; color:gray;'>You: {row['user_rating']:.1f} | Critic Avg: {row['critic_norm']:.1f}</span>", unsafe_allow_html=True)
with c2:
...
with t_genre:
    if 'genre_list' in df.columns:
        ...
        with cg1:
            ...
            st.dataframe(genre_stats.set_index('genre_list'), use_container_width=True)
        ...
        with cg2:
            ...
            fig = px.box(filtered_genre_df, x="genre_list", y="user_rating", color="genre_list")
            fig.update_layout(showlegend=False, xaxis_title="Genre", yaxis_title="Your Rating")
            st.plotly_chart(fig, use_container_width=True)
...
with t_rated:
    if 'rated' in df.columns:
        ...
        with cr1:
            fig = px.bar(rated_stats, x='rated', y='Count', hover_data={'Avg_Rating': ':.3f'}, title="Maturity Rating Breakdown")
            fig.update_traces(marker_line_width=1, marker_line_color="black")
            st.plotly_chart(fig, use_container_width=True)
        with cr2:
            fig = px.box(df, x="rated", y="user_rating", color="rated", title="Rating Spread per Maturity Level")
...
try:
    ...
    with t_sent:
        ...
        with c_pie:
            fig = px.pie(mood_counts, values='Count', names='Mood', title="VADER Sentiment Split", hole=0.4)
            fig.update_traces(marker_line_width=1, marker_line_color="black")
            st.plotly_chart(fig, use_container_width=True)
