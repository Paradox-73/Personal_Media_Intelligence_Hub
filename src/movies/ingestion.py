...
def get_movie_metadata(title, year, tmdb_id=None, imdb_id=None):
    ...
    if target:
        ...
        # Safely get credits
        credits = getattr(target, 'credits', None)
        ...
        tmdb_data = {
            ...
            'cast': get_names(getattr(credits, 'cast', [])),
            'director': [c['name'] for c in getattr(credits, 'crew', []) if c['job'] == 'Director'] if credits else [],
            'writer': [c['name'] for c in getattr(credits, 'crew', []) if c['department'] == 'Writing'] if credits else [],
...
def sync_letterboxd_data():
    """
    Syncs the raw ratings.csv and liked.csv with enriched_data.csv.
    - Updates user_rating and is_liked for existing movies in enriched_data.csv.
    - Adds truly new movies to enriched_data.csv after fetching metadata.
    """
    ...
    rename_map = {'Name': 'Name', 'Year': 'Year', 'Rating': 'Rating', 'Letterboxd URI': 'Letterboxd URI', 'Date': 'Date'}
    ...
    print(f"📊 Raw: {len(df_raw)} | Enriched: {len(df_enriched)}")
    ...
    for _, row in df_raw.iterrows():
        ...
        if uri in enriched_uris:
            ...
            if 'rating_date' in df_enriched.columns:
                if df_enriched.loc[mask, 'rating_date'].iloc[0] != date:
                    df_enriched.loc[mask, 'rating_date'] = date
                    changed = True
            else:
                df_enriched.loc[mask, 'rating_date'] = date
                changed = True
        ...
        # Truly New
            ...
            lb_name = str(row['Name'])
            lb_year = clean_year(row['Year'])
            print(f"✨ New Movie Found: {lb_name} ({lb_year})")
            
            metadata = get_movie_metadata(lb_name, lb_year)
            metadata['letterboxd_name'] = lb_name
            metadata['letterboxd_uri'] = uri
            metadata['user_rating'] = rating
            metadata['rating_date'] = date
            metadata['is_liked'] = is_liked
            metadata['year'] = lb_year # Ensure year is preserved even if API fails
    ...
    if updates_enriched > 0:
        ...
        print(f"✅ Updated {updates_enriched} existing movies in enriched_data.csv")

    if new_entries:
        df_new_to_add = pd.DataFrame(new_entries)
        df_enriched = pd.concat([df_enriched, df_new_to_add], ignore_index=True)
        df_enriched.to_csv(enriched_path, index=False)
        print(f"🚀 Added {len(new_entries)} new movies to enriched_data.csv")
    else:
