...
def train_models():
    print("🤖 Starting Game Model Training...")
    ...
    # --- 1. Regressor (Small dataset -> Conservative parameters) ---
    ...
    # Use XGB as primary if it performs better or similar
    ...
    print("\n" + "="*50)
    print("📊 PERFORMANCE REPORT: Game XGBoost Regressor")
    print("="*50)
    ...
    sns.kdeplot(preds_full, label='Predicted (Whole Data)', color='blue', linestyle='--')
    plt.title("Game Rating Distribution (KDE) - Whole Data")
    plt.xticks(np.arange(0, 5.5, 0.5))
    plt.legend()
    plt.savefig(results_dir / "games_distribution_kde.png")
    plt.close()
    ...
    sns.histplot(preds_full, bins=np.arange(-0.25, 5.75, 0.5), ax=axes[1], color='blue')
    axes[1].set_title("Predicted Ratings (Whole Data)")
    for ax in axes:
        ax.set_xticks(np.arange(0, 5.5, 0.5))
    plt.suptitle("Game Rating Histograms")
    plt.savefig(results_dir / "games_distribution_histograms.png")
    plt.close()

    # --- 3. Classifier (Optional but good for dashboard) ---
    ...
    # Save Models
    joblib.dump(regressor, config.GAMES_MODEL_REGRESSOR)
    joblib.dump(classifier, config.GAMES_MODEL_CLASSIFIER)
    
    # Save a CSV with predictions for the dashboard
    df_results = pd.DataFrame({
        'name': pd.read_csv(config.GAMES_ENRICHED_DATA_PATH)['name'],
        'actual': y_reg,
